...
def search_nba_players(query: str) -> list[dict]:
    ...
    try:
        ...
        return [p for p in all_players if q in p["name"].lower()][:15]
    except Exception:
        # Fallback: Basketball‑Reference alphabetical pages
...
def fetch_nba_player_logs(
    ...
    # Numeric IDs → stats.nba.com; slug IDs → Basketball‑Reference
    if str(player_id).isdigit():
        return _nba_logs_statscom(player_id, player_name, seasons)
    return _nba_logs_bref(player_id, player_name, seasons)
...
def default_recent_seasons(sport: str, count: int = 5) -> list[str]:
    sport = sport.lower()
    if sport in {"nba", "nhl"}:
        # Current date: April 2026 → 2025-26 season is active
