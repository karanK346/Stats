...
app = FastAPI(title="Player Line Predictor V2.6.1")
...
async def home(request: Request):
    return templates.TemplateResponse(
        "index.html", {"request": request, "stat_map": STAT_MAP}
    )
...
async def api_search(payload: SearchPayload):
    try:
        results = search_players(payload.sport.lower(), payload.query)
    ...
    try:
        ...
        if df.empty:
            raise HTTPException(
                status_code=404,
                detail=(
                    "No game logs found for that player / seasons. "
                    "Try adding older seasons or verify the player ID."
                ),
            )
...
async def api_predict(payload: PredictPayload):
    ...
    if payload.stat not in STAT_MAP.get(sport, []):
        raise HTTPException(
            status_code=400,
            detail=f"Stat '{payload.stat}' not supported for {sport}.",
        )

    df = get_player_logs(sport, payload.player_id)

    # Auto-sync if no cached data exists
    if df.empty and payload.auto_sync:
        seasons = payload.seasons or default_recent_seasons(sport, 5)
        player_name = payload.player_name or payload.player_id
        try:
            if sport == "nba":
                fresh = fetch_nba_player_logs(payload.player_id, player_name, seasons)
            elif sport == "nhl":
                fresh = fetch_nhl_player_logs(payload.player_id, player_name, seasons)
            elif sport == "mlb":
                fresh = fetch_mlb_player_logs(payload.player_id, player_name, seasons)
            else:
                raise HTTPException(status_code=400, detail="Unsupported sport.")
    ...
    if df.empty:
        raise HTTPException(
            status_code=404,
            detail=(
                "No cached data for this player and auto-sync returned nothing. "
                "Try syncing more seasons manually."
            ),
        )
